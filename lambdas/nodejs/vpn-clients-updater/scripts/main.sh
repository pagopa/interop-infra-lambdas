...
case $1 in
  create-client)
    CLIENT_NAME=$2
    CLIENT_EMAIL=$3
    DEFAULT_CREDENTIALS_DURATION_DAYS=$4
    
    EASYRSA_PATH=$EASYRSA_PATH EASYRSA_PKI_DIR=$EASYRSA_PKI_DIR CLIENT_NAME="$CLIENT_NAME" CLIENT_EMAIL="$CLIENT_EMAIL" EASYRSA_CERT_EXPIRE=$DEFAULT_CREDENTIALS_DURATION_DAYS EASYRSA_BATCH_MODE=1 $SCRIPTS_PATH/create_client.sh
